...
for ss=1:length(list_subj)
    ...
    for rr=1:length(list_run)
        ...
        switch subj_run
               case 'motLR'
                   subj_onset = [root_path filesep 'fmri_preprocess_all_tasks_niak-fix-scrub_900R/EVs' filesep subject filesep 'MOTOR_LR/MOTOR_LR_spm_onset.csv' ];
               case 'motRL'
                   subj_onset = [root_path filesep 'fmri_preprocess_all_tasks_niak-fix-scrub_900R/EVs' filesep subject filesep 'MOTOR_RL/MOTOR_RL_spm_onset.csv' ];
        end
        if ~exist(subj_onset)
    ...
    if ~isfield( files_in.fmri.(subject).sess1,'motLR') && ~isfield( files_in.fmri.(subject).sess1,'motRL')
